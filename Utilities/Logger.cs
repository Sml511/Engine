...
namespace PrimalEditor.Utilities
{
   ...
   class LogMessage 
    ...
    static class Logger
    {
        private static int _messagedFilter = (int)(MessageType.Info | MessageType.Warning | MessageType.Error);
        private static ObservableCollection<LogMessage> _messages = new ObservableCollection<LogMessage>();

        public static ReadOnlyCollection<LogMessage> Messages
        { get; } = new ReadOnlyCollection<LogMessage>(_messages);
        ...
        static Logger() 
        {
            FilteredMessages.Filter += (s, e) =>
            {
                ...
                e.Accepted = (type & _messagedFilter) != 0;
